...
[config.json](../config.json) owns service locations, camera defaults, queue/lease limits, source timing, controller bounds and output cadence. The launcher uses the configured sibling component directories and each component's runtime. A root clone includes Keyboard and Light Track source code; their Python environments and local models must be provisioned separately.
