...
Entries preserve the evidence for their stated version. Current operation is described in the [README](../README.md) and [architecture](architecture.md).

## 2026-09-22 — Document session, source and controller boundaries

Separated camera lifecycle, measurement selection and display-controller states in the architecture. Added startup/failure behavior, capture-time freshness, bounded storage, model activation and temporary-anchor constraints. Consolidated the earlier iteration log here and retained its old path as a link. The README now states sibling-repository prerequisites and distinguishes measurement, target and display output.

Verification: all 28 existing Fusion tests pass with process-local IPv4-first DNS; all four renderer CTest suites pass. Documentation links and preserved historical entries were checked. This update changes documentation only and provides no new physical-camera acceptance.
