...
| Light Track Node suite | 38 passing |
| Light Track Python training suite | 17 passing |
| Fusion Node suite | 27 passing |
...
Observed smoke results: model-output startup 80°, authoritative keyboard 25°, subsequent brightness fallback 25°, 15 accepted representative anchors, 47 recorded feature frames, and zero displayed speed-bound violations. The exact display sample count varies with scheduling. Generated outputs are ignored under `fusion/data/smoke/` and marked synthetic.
...
## 0.2.0 sweep profiles and deadline controller

See [sweep validation](sweep-validation.md). The current controller replaces the historical 1°/s rest limit with septic corrections and an immutable one-second chase deadline. `node research/controller-validation.mjs` generates synthetic trajectories; `npm test` includes the real Python training/profile API round trip. These do not substitute for a physical laptop sweep.
