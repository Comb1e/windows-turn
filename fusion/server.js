...
function accept(s,kind,result){
  if(session!==s)return;
  s.errors[kind]=null;s.engine.ingest(kind,result);
...
function start(camera){
  ...
  for(const kind of ['keyboard','lighting'])s.clients[kind]=new ServiceClient(kind,config.services[kind],camera,config.network,
    result=>accept(s,kind,result),error=>{if(session===s){s.errors[kind]=error.message;s.engine.invalidate(kind);}});
...
const interval=setInterval(()=>{
  ...
  latest={sessionId:s.id,...s.engine.tick(now),services:{keyboard:s.errors.keyboard??null,lighting:s.errors.lighting??null},
    droppedFrames:Object.fromEntries(Object.entries(s.clients).map(([kind,client])=>[kind,client.queue.dropped]))};
  if(s.recording){
...
const server=createServer(async(req,res)=>{
  ...
  try{
    ...
    if(path==='/api/health'&&req.method==='GET'){send(200,{services:await health(),config});return;}
    ...
    if(path==='/api/start'&&req.method==='POST'){
      ...
      await json(req);const services=await health();
      if(session)fail('A camera session is already active',409);
      ...
      const s=start(camera);send(200,{sessionId:s.id,camera,services});return;
    }
...
for(const signal of ['SIGINT','SIGTERM'])process.once(signal,async()=>{clearInterval(interval);await stop();for(const res of listeners)res.end();server.close();});
