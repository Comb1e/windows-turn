...
export class ServiceClient {
  constructor(kind,url,camera,settings,onResult,onError){Object.assign(this,{kind,url,camera,settings,onResult,onError});this.id=null;this.closed=false;this.retryAt=0;
  ...
  async connect(){
    ...
    try{
      const result=await requestJson(this.url,'/v1/sessions',{method:'POST',body:{camera:this.camera},timeoutMs:this.settings.requestTimeoutMs});
      if(this.closed){await requestJson(this.url,`/v1/sessions/${result.sessionId}`,{method:'DELETE'});return false;}
      this.id=result.sessionId;this.info=result;return true;
  ...
  async process(frame){
    ...
    try{
      ...
      if(!this.closed&&this.id===id&&result.sessionId===id&&result.frameId===frame.frameId&&result.timestampMs===frame.timestampMs)this.onResult(result,frame);
    } catch(error){
