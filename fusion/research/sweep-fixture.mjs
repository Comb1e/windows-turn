...
export function sweep({speed=10,closeSpeed=10,hiddenSpeed=10,motion=true,manual=false,keyboardMax=44}={}){
  const keyboardModel={modelId:'keyboard-fixture',angleRange:[10,keyboardMax]};
  const cal=new SweepCalibration(config,{device:'test-laptop',location:'test-room',position:'desk',lighting:'lamp',display:'fixed'},keyboardModel);
  let t=0,id=0,previous=null;const frames=[];
  const frame=(angle,moving=false)=>{
    const keyboard={frameId:++id,timestampMs:t,valid:angle<=keyboardMax,angleDeg:angle<=keyboardMax?angle:null,modelId:keyboardModel.modelId};
  ...
  for(let angle=19;angle<=keyboardMax;angle+=speed/10)frame(angle,true);
  for(let angle=keyboardMax+hiddenSpeed/10;angle<120;angle+=hiddenSpeed/10)frame(angle,true);
