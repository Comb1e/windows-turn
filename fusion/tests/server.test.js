...
async function mock(kind){
  ...
  const server=createServer(async(req,res)=>{
    ...
    if(req.url==='/v1/health')result={ready:true,modelReady:true,camera:{width:64,height:64},angleRange:[10,44]};
      ...
      result={sessionId:`${kind}-${id}`,frameId:Number(req.headers['x-frame-id']),timestampMs:Number(req.headers['x-timestamp-ms']),angleDeg:angle,valid:angle!==null,quality:{},motion:{velocityDegS:0}};
...
test('coordinator HTTP sends identical RGBA frames and pairs anchors without blocking keyboard output',async()=>{
  const keyboard=await mock('keyboard'),lighting=await mock('lighting');
